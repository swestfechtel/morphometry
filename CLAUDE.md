...
- Both `extract_subregions` enforce subregion contiguity (`_remove_subregion_fragments`, keep 26-connectivity components ≥ 20% of the largest): a subregion is one anatomical patch, but the `(x,y)` `classify_point` split can bucket a few plateau-midline/edge voxels into a subregion disconnected from its main body (small `aLT`/`pRT` fragments at high flexion). No-op on clean scans (single-component subregions).
