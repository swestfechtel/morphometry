...
- Femoral central weight-bearing zone (CWBZ) extraction restricts to the tibia-facing superior-inferior cluster (`_restrict_to_contact_cluster`): the A-P/L-R window from the tibial plateau alone would, at high knee flexion, also capture the anterior trochlea folded into the same A-P range, so only the S-I cluster nearest the tibial contact is kept. Preserve this when touching `extract_central_weightbearing_zone`.
