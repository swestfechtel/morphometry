    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (resp.ok) {
                ...
                setError(t('loginFailed', { status: resp.status }));
            }
        } catch {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-8 rounded shadow-md w-full max-w-sm">
                <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-100">{t('signIn')}</h2>
                <label className="block mb-1 text-sm text-gray-700 dark:text-gray-300" htmlFor="username">{t('username')}</label>
                <input
                    id="username"
                    value={username}
                ...
                />
                <label className="block mb-1 text-sm text-gray-700 dark:text-gray-300" htmlFor="password">{t('password')}</label>
                <input
                    id="password"
                    type="password"
                    value={password}
